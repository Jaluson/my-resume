...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><ResumeManager resumes={store.resumes} selectedId={currentResume.id} onSelect={selectResume} onCreate={createResume} onDuplicate={duplicateResume} onRename={renameResume} onDelete={deleteResume} mobileOpen={mobileManagerOpen} onCloseMobile={() => setMobileManagerOpen(false)} /><section id="resume-editor-panel" className={`editor-column ${mobilePane === 'preview' ? 'mobile-hidden' : ''}`}><div className="column-heading"><div><p className="eyebrow">编辑内容</p><h1>{currentResume.title || '未命名简历'}</h1></div><span className="hint-text">所有修改自动保存</span></div><ResumeEditor resume={currentResume} onChange={updateResume} /></section><section id="resume-preview-panel" className={`preview-column ${mobilePane === 'edit' ? 'mobile-hidden' : ''}`}><div className="preview-heading"><div><p className="eyebrow">实时预览</p><h2>A4 版式</h2></div><span className="preview-scale">适配预览</span></div><div className="preview-stage"><ResumePreview resume={currentResume} ref={previewRef} /></div><div className="template-panel"><div className="template-panel-heading"><div><p className="eyebrow">版式风格</p><h3>选择模板</h3></div><span>即时应用</span></div><TemplatePicker value={currentResume.templateId} onChange={(templateId) => updateResume({ ...currentResume, templateId })} /><p className="hint-text">Word 导出保留内容与强调色，不复制现代模板的侧栏结构；需要视觉版式时请导出 PDF。</p></div></section></main>
