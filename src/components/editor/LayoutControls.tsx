...
export default function LayoutControls({ resume, onChange }: LayoutControlsProps) {
  ...
  useEffect(() => {
    ...
    const handleChange = () => setMobileOpen(!media.matches)
    handleChange()
  ...
  return <section className={`layout-controls ${mobileOpen ? 'is-open' : 'is-collapsed'}`} aria-labelledby="layout-controls-title">
    <button className="layout-controls-mobile-toggle" type="button" onClick={() => setMobileOpen(!mobileOpen)} aria-expanded={mobileOpen} aria-controls="layout-controls-content"><span><small>自由排版</small><strong>调整版式</strong></span>{mobileOpen ? <ChevronUp size={18} aria-hidden="true" /> : <ChevronDown size={18} aria-hidden="true" />}</button>
    <div className="layout-controls-content" id="layout-controls-content">
      ...
      <div className="layout-control-grid">
        ...
        <label className="field"><span>文字大小</span><div className="range-control"><input type="range" min="0.85" max="1.15" step="0.05" value={resume.layout.fontScale} aria-valuetext={`${Math.round(resume.layout.fontScale * 100)}%`} onChange={(event) => updateLayout({ fontScale: Number(event.target.value) })} /><output>{Math.round(resume.layout.fontScale * 100)}%</output></div></label>
        ...
        <label className="field"><span>区块间距</span><div className="range-control"><input type="range" min="8" max="36" step="1" value={resume.layout.sectionGap} aria-valuetext={`${resume.layout.sectionGap}px`} onChange={(event) => updateLayout({ sectionGap: Number(event.target.value) })} /><output>{resume.layout.sectionGap}px</output></div></label>
        <label className="field"><span>行高</span><div className="range-control"><input type="range" min="1.3" max="1.8" step="0.05" value={resume.layout.lineHeight} aria-valuetext={resume.layout.lineHeight.toFixed(2)} onChange={(event) => updateLayout({ lineHeight: Number(event.target.value) })} /><output>{resume.layout.lineHeight.toFixed(2)}</output></div></label>
