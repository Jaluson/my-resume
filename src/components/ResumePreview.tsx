  ...
  return (
    <div ref={forwardedRef} className={`resume-paper resume-${resume.templateId}`} data-resume-preview style={accentStyle}>
      <div className="resume-main">
        ...
        {hasText(content.summary) && <section className="resume-section"><h2>个人简介</h2>{text(content.summary, editing, (value) => onChange?.({ ...resume, summary: value }), 'p', '个人简介')}</section>}

        {content.experience.length > 0 && <section className="resume-section"><h2>工作经历</h2>{content.experience.map((item) => (
        ...
        {content.education.length > 0 && <section className="resume-section"><h2>教育经历</h2>{content.education.map((item) => (
        ...
        {content.skills.length > 0 && <section className="resume-section"><h2>技能</h2><div className="resume-tags">{content.skills.map((skill, index) => { const sourceIndex = resume.skills.indexOf(skill); return <span key={`${skill}-${index}`}>{editing ? <EditableText value={skill} onChange={(value) => editList('skills', sourceIndex, value)} as="span" ariaLabel={`技能 ${index + 1}`} /> : skill}</span> })}</div></section>}
