  ...
  return (
    <div ref={forwardedRef} className={`resume-paper resume-${resume.templateId} resume-custom-order resume-density-${resume.layout.density} resume-align-${resume.layout.alignment}`} data-resume-preview style={accentStyle}>
      <div className="resume-main">
        {(hasProfile || editing) && <header className="resume-header">
          <div>
            {(hasText(profile.fullName) || editing) && text(profile.fullName, editing, (value) => editProfile('fullName', value), 'h1', '姓名', '例如：张三')}
            {(hasText(profile.jobTitle) || editing) && text(profile.jobTitle, editing, (value) => editProfile('jobTitle', value), 'p', '职位', '例如：产品经理', 'resume-role')}
          </div>
          {(hasContact || editing) && <div className="resume-contact">
            {(hasText(profile.email) || editing) && (editing ? <EditableText value={profile.email} onChange={(value) => editProfile('email', value)} as="span" ariaLabel="邮箱" placeholder="邮箱" /> : <a href={`mailto:${profile.email}`}>{profile.email}</a>)}
            {(hasText(profile.phone) || editing) && text(profile.phone, editing, (value) => editProfile('phone', value), 'span', '电话', '电话')}
            {(hasText(profile.location) || editing) && text(profile.location, editing, (value) => editProfile('location', value), 'span', '所在地', '所在地')}
            {(hasText(profile.website) || editing) && (editing ? <EditableText value={profile.website} onChange={(value) => editProfile('website', value)} as="span" ariaLabel="个人网站" placeholder="个人网站" /> : websiteUrl ? <a href={websiteUrl} target="_blank" rel="noreferrer">{cleanUrl(profile.website)}</a> : <span>{cleanUrl(profile.website)}</span>)}
          </div>}
        </header>}

        {!hasContent && <div className="resume-empty">在左侧编辑内容，预览会即时出现在这里</div>}
        {isVisible('summary') && hasText(content.summary) && <section className="resume-section" style={{ order: resume.layout.sectionOrder.indexOf('summary') + 1 }}><h2>{sectionTitle('summary')}</h2>{text(content.summary, editing, (value) => onChange?.({ ...resume, summary: value }), 'p', '个人简介')}</section>}

        {isVisible('experience') && content.experience.length > 0 && <section className="resume-section" style={{ order: resume.layout.sectionOrder.indexOf('experience') + 1 }}><h2>{sectionTitle('experience')}</h2>{content.experience.map((item) => (
        ...
        {isVisible('education') && content.education.length > 0 && <section className="resume-section" style={{ order: resume.layout.sectionOrder.indexOf('education') + 1 }}><h2>{sectionTitle('education')}</h2>{content.education.map((item) => (
        ...
        {isVisible('skills') && content.skills.length > 0 && <section className="resume-section" style={{ order: resume.layout.sectionOrder.indexOf('skills') + 1 }}><h2>{sectionTitle('skills')}</h2><div className="resume-tags">{content.skills.map((skill, index) => { const sourceIndex = resume.skills.indexOf(skill); return <span key={`${skill}-${index}`}>{editing ? <EditableText value={skill} onChange={(value) => editList('skills', sourceIndex, value)} as="span" ariaLabel={`技能 ${index + 1}`} /> : skill}</span> })}</div></section>}

        {isVisible('projects') && content.projects.length > 0 && <section className="resume-section" style={{ order: resume.layout.sectionOrder.indexOf('projects') + 1 }}><h2>{sectionTitle('projects')}</h2>{content.projects.map((item) => {
