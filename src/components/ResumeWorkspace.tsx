    ...
    <div className="workbench-heading">
      ...
      <div className="workbench-heading-side">
        ...
        <div className="mobile-switcher" role="tablist" aria-label="切换工作区面板">
          <button id="resume-editor-tab" className={mobilePane === 'editor' ? 'is-active' : ''} type="button" role="tab" aria-selected={mobilePane === 'editor'} aria-controls="resume-editor-panel" onClick={() => setMobilePane('editor')}><PencilLine size={15} aria-hidden="true" />编辑内容</button>
          <button id="resume-preview-tab" className={mobilePane === 'preview' ? 'is-active' : ''} type="button" role="tab" aria-selected={mobilePane === 'preview'} aria-controls="resume-preview-panel" onClick={() => setMobilePane('preview')}><Eye size={15} aria-hidden="true" />实时预览</button>
        </div>
      </div>
    </div>
